...
class TestYOLODetection:
    ...
    def test_format_to_list_output(
        self, yolo_detection, mock_detection_data, expected_formatted_detections
    ):
        result = yolo_detection.format_to_list_output(mock_detection_data)
        assert result == expected_formatted_detections
        assert len(result) == 3

    def test_format_to_list_output_empty(
        self, yolo_detection, mock_detection_data_empty
    ):
        result = yolo_detection.format_to_list_output(mock_detection_data_empty)
        assert result == []
        assert len(result) == 0

    def test_format_to_list_output_cars_only(
        self, yolo_detection, mock_detection_data_cars_only
    ):
    ...
    def test_detect_success(
        ...
        with patch.object(
            yolo_detection.model, "predict", return_value=mock_detection_data
        ):
